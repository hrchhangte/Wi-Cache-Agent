...
void CacheAgentMain::readConfigFile(){
	...
	string fileName = "agentConfigFile";

	//read from the cache/file
	string filePath = "/mnt/app/";
	...
	if(fs.is_open()){
		...
		while (getline(fs, line )) {
		   ...
		   // cout<<line<<endl;
			...
			if(tokens[0].compare("serverAddr")==0){
				//obtain the server's IP address
				ctrIp = tokens[1];
			}
			else if(tokens[0].compare("serverPort")==0){
				//obtain the server's port no.
				ctrPort = tokens[1];
			}
		}//while
...
void CacheAgentMain::getCtrMesg(){
	...
	while(true){
		...
		if(tokens[0].compare("DELIVER_CHUNK")==0){

			//Message format : DELIVER_CHUNK <CONNECTION_KEY> <CHUNK_NAME> <CHUNK_SIZE> <SKIP_BYTES> <CLIENT_KEY> <CLIENT_PORT>
													//<NEIGHBOR_IP = NULL IF NOT NEEDED>

			param[i].threadId = i;
			param[i].connKey =  atoi((char *)tokens[1].c_str());
			param[i].chunkName = tokens[2];
			param[i].chunkSize = atol((char *)tokens[3].c_str());
		...
		else if(tokens[0].compare("REQ_TRANSFR_STAT")==0){

			//message format : REQ_TRANSFR_STAT <client ip>
			//response format : TRANSFR_STAT <client ip> <finish status> <file name> <file size> <bytes sent>

			stringstream fileSizess, bytesSentss;
			fileSizess<<threadMap[tokens[1]].fileSize;
			bytesSentss<<threadMap[tokens[1]].bytesSent;

			transfrStat = "TRANSFR_STAT";
...
void CacheAgentMain::getCtrMesg(){
	...
	while(true){
		...
		if(tokens[0].compare("SEND_FILE")==0){

			//new message format : SEND_FILE <file name> <file size> <skip bytes> <client ip>  <cached stat> <neighbor AP>
			param[i].threadId = i;
			...
			param[i].finTrans = "false"; //not used
			param[i].handOver = "false"; //not used
			param[i].ctrSockHndle = ctrSockHndle; //for writing back to controller
			param[i].clntPort = "9001";atol((char *)tokens[4].c_str());
...
/*
 * send file/chunk/segment data to the client
 */
...
void CacheAgentMain::sndFileTcp(string mapKey){
	...
	string clntIp = wifiConnMap[clientKeyss.str()].clientIP; //get IP using the clientKey
	string chunkName = contReqMap[mapKey].chunkName;
	long chunkSize = contReqMap[mapKey].chunkSize;
	...
	string::size_type sz;
	stringstream chunkSizess, skipBytesss, connKeyss;
	...
	*/
	...
	mesg=mesg+" "+connKeyss.str()+" "+chunkName.c_str()+" "+"NULL";
	...
	//send chunk head to client
	mesg = "CHUNK_HEAD";
	mesg=mesg+" "+connKeyss.str()+" "+chunkName+" "+chunkSizess.str()+" "+skipBytesss.str();
	mesg += '\n';
	...
	if((read(clntSockHndle, recvChar, sizeof(recvChar)))==-1){
		...
		cout<<"[Thread - "<<threadId<<"] "<<"sendFileTcp: Message received from controller!"<<endl;
	...
	if(tokens[0].compare("CHUNK_ACK")==0){

		long chunkAckValue = atol((char *)tokens[3].c_str());

		contReqMap[mapKey].skipBytes = chunkAckValue;
		skipBytes = chunkAckValue; //update skipBytes using data from client
	...
	*/
	...
	//static char recvChar[BUFSIZ];
		...
		//send chunkAck to controller
		stringstream chunkAckss;
		chunkAckss<<contReqMap[mapKey].chunkAck;
		mesg = "CHUNK_ACK";
		mesg=mesg+" "+connKeyss.str()+" "+chunkName+" "+chunkAckss.str();
...
void CacheAgentMain::sndFileTcp(string mapKey){
	...
	int threadId = threadMap[mapKey].threadId;
	string clntIp = threadMap[clientKey].clientIP; //get IP using the clientKey
...
void CacheAgentMain::udpUpdate(string mapKey, int updateCode){
	...
	int ctrUpdateSock;
	const char *ctrAddr =  "192.168.1.145";
	const char *ctrUdpPort = "7000";
...
void CacheAgentMain::sndUpdate(string mapKey){
	...
	int ctrUpdateSock;
	const char *ctrAddr =  "192.168.1.2";
	const char *ctrUdpPort = "7000";
	struct addrinfo ctrUpdateAddrInfo;
	struct addrinfo *ctrUpdateAddrInfoList;
...
void CacheAgentMain::sndFileTcpMobility(string mapKey){
	...
	if(bytesSent>0){
		...
		if((tokens[0].compare("FILE_UPDATE")==0) && (tokens[1].compare(fileName.c_str())==0)
