 ...
 * thread that handles the sending of chunk/segment
 * to a cliet
 */
struct threadParam{
	int threadId; // thread id
	string clntIp; // the client IP address to which chunk/segment will be sent
	string fileName; //the chunk/segment name that is to be sent
	long fileSize; //the size of chunk/segment
	...
	const char *clntPort; //the port number where client is expecting data of chunk/segment
	int clntSockHndle; //the socket handle for the connection to the client
	string cachedStat; //whether the chunk/segment is to be cached or not
 ...
 * the client to which the chunk/segment
 * is to be delivered
 *
 */
struct contReqParam{
	...
	string chunkName; //the chunk/segment name
	long chunkSize; //the size of the chunk/segment
	long skipBytes; //the number of bytes to be skipped, i.e, byte index
	int clientKey; //the key of the client to which chunk will be sent
	long chunkAck; //the ack for the chunk/agent from the client
...
static unordered_map <string, struct contReqParam> contReqMap; //map of the client IP address and the connection object
